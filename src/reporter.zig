...
pub fn GenReporter(comptime Item: type, comptime Report: type) type {
    return struct {
        ...
        fn display(self: *const Self, report: *const GenReport(Report), file_name: []const u8) !void {
            ...
            //  56 | var a = 3
            if (previous_line) |pl| {
                try self.print_line(line_count - 1, pl, line_digit_count, self.writer);
            }

            // Prints current line number, separation and line
            //  57 | fn add(a, b c)
            try self.print_line(line_count, self.source[line_start..current], line_digit_count, self.writer);
        ...
        fn print_line(self: *Self, line_nb: usize, line: []const u8, digit_count: usize) !void {
